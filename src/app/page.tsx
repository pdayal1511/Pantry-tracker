  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (querySnapshot) => {
      ...
      const calculateTotal = () => {
        const totalAmount = itemsArr.reduce((sum, item) => sum + parseFloat(item.amount), 0)
    ...
    <main className="flex min-h-screen flex-col items-center justify-between sm:p-24 p-4">
      ...
      <div className='bg-slate-800 p-4 rounded-lg'>
        <form className='grid grid-cols-6 items-center text-black'>
          ...
          <input value={newItem.amount} onChange={(e) => setNewItem({...newItem, amount: e.target.value })} className='col-span-2 p-3 border mx-3' type="number" placeholder='Quantity' />
        ...
        <ul>
          {items.map((item, id) => (
            <li key={id} className='my-4 w-full flex justify-between bg-slate-950'>
              ...
              <button onClick={() => deleteItem(item.id)} className='text-white ml-8 p-4 border-l-2 border-slate-900 hover:bg-slate-900 w-16'>X</button>
            </li>
          ))}
        </ul>
        {items.length < 1 ? ('') : (
